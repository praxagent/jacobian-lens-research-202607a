...
2. **What does resuming from the *published fp16* lens cost?** Only that lens's own fp16 storage rounding — **measured**: a fair fp16-vs-fp16 comparison puts extended-vs-continuous at **2.8×10⁻⁴**, indistinguishable from the fp16 round-trip floor itself (2.3×10⁻⁴), and it is weighted 24/n so it shrinks further as you extend. Every Jacobian lens ships in fp16 anyway (ours, Neuronpedia's), so this is the format's floor, not a defect of extending. A from-scratch refit removes even that; it is a purity preference, not a correctness fix.
...
None of this would exist without other people's open-sourcing. Anthropic released the Jacobian lens — the method, the math, and a working `jlens` implementation — under Apache-2.0; Neuronpedia built and open-sourced the fitting pipeline and a whole collection of pre-fitted lenses up to ~70B. We are a small, self-funded independent lab, and our part is modest: we took their open tools and spent a few hundred dollars pushing one lens onto a larger model than the public collection covered, then audited it carefully. That we could do it at all is a direct dividend of their decision to open-source, and the right response is to pay it forward — so this lens is public, every run is receipted down to the dollar, the discriminating experiments reproduce on one A100 for about a dollar, and the warm-start tooling lets anyone continue our n=24 average toward convergence without refitting. The n=24 lens is a valid instrument for the contrast experiments here (see *Why n=24 is enough*) and an open starting point for whoever wants a longer average; the checkpoint math is exact and the commands are above.
